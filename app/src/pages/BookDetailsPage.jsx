...
function BookDetailsPage() {
  ...
  useEffect(() => {
    const fetchBookDetails = async () => {
      try {
        ...
        const response = await axios.get(`https://www.googleapis.com/books/v1/volumes/${googleBookId}?key=${API_KEY}`);
  ...
  if (loading) {
    return <div className="text-center mt-10 text-white">Carregando detalhes do livro...</div>;
  }

  if (!book) {
    return <div className="text-center mt-10 text-white">Livro não encontrado.</div>;
  }

  return (
    <div className="bg-gray-900 text-gray-200 min-h-screen p-8 font-sans">
      <header className="flex justify-between items-center mb-12">
        <h1 className="text-4xl md:text-5xl font-extrabold text-white tracking-tight">Detalhes do Livro</h1>
      ...
      <div className="flex flex-col md:flex-row items-center md:items-start gap-12 bg-gray-800 p-8 rounded-lg shadow-xl">
        ...
        <div className="flex-grow text-left">
          ...
          <div className="text-sm text-gray-500 mb-6">
            <p><span className="font-semibold">Publicado em:</span> {book.publishedDate}</p>
            <p><span className="font-semibold">Editora:</span> {book.publisher}</p>
            <p><span className="font-semibold">Páginas:</span> {book.pageCount}</p>
          </div>
          <h3 className="text-xl font-semibold text-white mb-2">Descrição:</h3>
          <p className="text-gray-400 leading-relaxed" dangerouslySetInnerHTML={{ __html: book.description }}></p>
        </div>
      </div>
    </div>
  );
